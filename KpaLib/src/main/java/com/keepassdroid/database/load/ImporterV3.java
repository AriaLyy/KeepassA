...
public class ImporterV3 extends Importer {
   ...
   * {@link InvalidKeyFileException}
   * {@link InvalidPasswordException}
   * {@link InvalidDBSignatureException}
   * {@link InvalidDBVersionException}
   * {@link IllegalBlockSizeException} on a decryption error, or possible internal bug.
   * {@link BadPaddingException} on a decryption error, or possible internal bug.
   * {@link NoSuchAlgorithmException} on a decryption error, or possible internal bug.
   * {@link NoSuchPaddingException} on a decryption error, or possible internal bug.
   * {@link InvalidAlgorithmParameterException} if error decrypting main file body.
   * {@link ShortBufferException} if error decrypting main file body.
   */
  public PwDatabaseV3 openDatabase(InputStream inStream, String password, InputStream kfIs)
      throws IOException, InvalidDBException {
    return openDatabase(inStream, password, kfIs, 0);
  }
  ...
  public PwDatabaseV3 openDatabase(InputStream inStream, String password, InputStream kfIs,
    ...
    byte[] filebuf = new byte[fileSize
        + 16]; // Pad with a blocksize (Twofish uses 128 bits), since Android 4.3 tries to write more to the buffer
    ...
    try {
      cipher.init(Cipher.DECRYPT_MODE, new SecretKeySpec(newManager.finalKey, "AES"),
          new IvParameterSpec(hdr.encryptionIV));
    ...
    try {
      encryptedPartSize =
          cipher.doFinal(filebuf, PwDbHeaderV3.BUF_SIZE, fileSize - PwDbHeaderV3.BUF_SIZE, filebuf,
              PwDbHeaderV3.BUF_SIZE);
   ...
   */
  void readGroupField(PwDatabaseV3 db, PwGroupV3 grp, int fieldType, byte[] buf, int offset)
      throws UnsupportedEncodingException {
    switch (fieldType) {
      case 0x0000:
        // Ignore field
