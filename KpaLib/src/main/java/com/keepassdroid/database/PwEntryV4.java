...
public class PwEntryV4 extends PwEntry implements ITimeLogger {
  private final String TAG = "PwEntryV4";
  ...
  /**
   * 保存时，取的是 {@link PwDatabaseV4#customIcons}
   * @deprecated 用于提示，保存新的icon时，需要将icon的一份引用存到{@link PwDatabaseV4#customIcons}
   */
  @Deprecated
  public void setCustomData(PwCustomData customData) {
  ...
  /**
   * 自定义字段，一般是插件使用的
   * 保存时，取的是 {@link PwDatabaseV4#customData}
   * @deprecated 用于提示，保存新的custom时，需要将icon的一份引用存到{@link PwDatabaseV4#customData}
   */
  @Deprecated
  public void setCustomIcon(PwIconCustom customIcon) {
    this.customIcon = customIcon;
  }

  @Override public boolean equals(Object obj) {
    if (!(obj instanceof PwEntryV4)) {
      return false;
    }
    PwEntryV4 other = (PwEntryV4) obj;
    // 已重写了ProtectedString的方法，可以直接使用equals判断
    return strings.equals(other.strings)
        && binaries.equals(other.binaries)
