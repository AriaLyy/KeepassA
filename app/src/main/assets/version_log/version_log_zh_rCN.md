...
* new: 浏览器与应用自动填充 TOTP 字段
* new: 输入法键盘支持条目搜索
* new: 设置新增「已适配浏览器」入口,按引擎分组展示
* new: 版本日志支持德语、俄语、繁体中文
* opt: 详情页卡片与搜索弹窗夜间模式适配
* opt: WebDAV 在 HTTP/2 StreamReset 后自动清理连接池
* fix: 修复 #117 群组条目首字母排序失效
* fix: 修复 Edge/Chrome 浏览器自动填充启动失败
* fix: 修复 MI Browser 聚焦 URL/搜索栏时无填充 UI
* fix: WebDAV 网络故障时跳过远程回滚,避免数据丢失
* fix: 多 TOTP 候选时按置信度优先,丢弃仅匹配验证码的字段
* fix: 修复工具栏副标题裁剪
