 ...
 */
package com.lyy.keepassa.widgets.arc.animation;
...
public class DefaultAnimationHandler extends MenuAnimationHandler {
    ...
    public void animateMenuOpening(Point center) {
        ...
        for (int i = 0; i < menu.getSubActionItems().size(); i++) {
            ...
            animation.setInterpolator(InterpolatorConstance.INSTANCE.getEaseInCubic());
    ...
    public void animateMenuClosing(Point center) {
        ...
        for (int i = 0; i < menu.getSubActionItems().size(); i++) {
            PropertyValuesHolder pvhX = PropertyValuesHolder.ofFloat(View.TRANSLATION_X,
                - (menu.getSubActionItems().get(i).x - center.x + (float) menu.getSubActionItems()
                .get(i).width / 2));
            PropertyValuesHolder pvhY = PropertyValuesHolder.ofFloat(View.TRANSLATION_Y,
                - (menu.getSubActionItems().get(i).y - center.y + (float) menu.getSubActionItems()
                .get(i).height / 2));
            PropertyValuesHolder pvhR = PropertyValuesHolder.ofFloat(View.ROTATION, -720);
            ...
            final ObjectAnimator animation = ObjectAnimator.ofPropertyValuesHolder(menu.getSubActionItems().get(i).view, pvhX, pvhY, pvhR, pvhsX, pvhsY, pvhA);
            animation.setDuration(DURATION);
            animation.setInterpolator(InterpolatorConstance.INSTANCE.getEaseOutCubic());
