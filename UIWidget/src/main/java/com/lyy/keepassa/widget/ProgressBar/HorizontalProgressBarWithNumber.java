...
public class HorizontalProgressBarWithNumber extends ProgressBar {
  ...
  private void obtainStyledAttributes(AttributeSet attrs) {
    ...
    showPercent =
        attributes.getBoolean(R.styleable.HorizontalProgressBarWithNumber_progress_show_percent,
            true);
  ...
  protected synchronized void onDraw(Canvas canvas) {
    ...
    // draw reached bar
    float endX = progressPosX - (mTextOffset >> 1);
    if (endX > 0) {
    ...
    // draw unreached bar
    if (!noNeedBg) {
      float start = progressPosX + (mTextOffset >> 1) + textWidth;
