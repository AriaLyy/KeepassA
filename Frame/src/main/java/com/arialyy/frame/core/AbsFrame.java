...
public class AbsFrame {
  ...
  private CopyOnWriteArrayList<AbsActivity> mActivityStack = new CopyOnWriteArrayList<>();
  ...
  private AbsFrame(Application application) {
    ...
    FrameApp.context = mContext;
    FrameApp.app = application;
  }
   ...
   * 获取Activity栈
   */
  public CopyOnWriteArrayList<AbsActivity> getActivityStack() {
   ...
   * activity是否已经启动
   *
   * @return true activity 已经启动
   */
  public boolean activityIsRunning(Class clazz) {
    if (mActivityStack.size() == 0) {
      return false;
    }
    for (AbsActivity ac : mActivityStack) {
